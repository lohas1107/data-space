...
本架構採 Consumer 直接查詢 Provider 的受保護目錄。Provider 以 DCP 驗證身分與所需資格，再對照政策決定回傳內容。下列兩張循序圖分別呈現準備工作與受保護查詢，各自從 1 開始編號。
...
| Provider | Security Token Service、Decentralized Identifier Service | 產生 VP 查詢用身分 token，提供 Provider 的 DID 文件。 |
| Consumer | Control Plane、Security Token Service | 探索版本及端點，取得身分 token 並提出目錄查詢。 |
| Consumer | Decentralized Identifier Service、Credential Service | 提供 DID 文件及 CS 端點；驗證 VP 查詢者後出示允許的 VP。 |
...
sequenceDiagram
    ...
    box rgb(253,250,246) Provider
        ...
        participant PDID as Decentralized<br/>Identifier Service
        participant P as Control Plane
    end
    box rgb(253,250,246) Consumer
        participant C as Control Plane
        participant CSTS as Security Token<br/>Service
        participant CDID as Decentralized<br/>Identifier Service
...
- **Holder 與 Verifier**：此處 Consumer 為 Holder、Provider 為 Verifier。Provider 查詢 Consumer 的 CS 時，使用 **Provider 簽署**的身分 token。若 Consumer 提供 VP 存取 token，Provider 依規則將它放入該身分 token 的 `token` claim。CS 驗證查詢者身分及出示權限；不把內部權限檢查畫成固定 CS → STS API。[DCP VPP](https://github.com/eclipse-dataspace-dcp/decentralized-claims-protocol/blob/v1.0.1/specifications/verifiable.presentation.protocol.md)
...
- **協定選擇**：DSP 未限定採用 DCP；本架構選用 DCP 保護目錄查詢。DCP 也可用於後續合約或傳輸控制互動，無須將每次驗證理解為重新簽發 VC。[DCP 與 DSP 的關係](https://github.com/eclipse-dataspace-dcp/decentralized-claims-protocol/blob/v1.0.1/specifications/dataspace.ecosystem.md#interrelation-to-the-dataspace-protocol)
- **解析與支援服務**：DID Resolution 依 DID method 實現。Catalogue Services 可支援聚合與發現，本圖採直接查詢，不要求它介入。圖中呈現成功查詢；拒絕、錯誤、重試與服務內部驗證不另增圖。
